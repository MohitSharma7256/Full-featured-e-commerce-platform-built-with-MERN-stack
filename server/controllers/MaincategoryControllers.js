...
async function createRecord(req, res) {
    console.log('Request files:', req.file) // Check if file is received
    console.log('Request body:', req.body)  // Check other fields
    try {
        let data = new Maincategory({
            ...req.body,
            pic: req.file ? req.file.path : undefined
        })
        console.log('Data to save:', data) // Check before save
        await data.save()
        console.log('Saved data:', data)   // Check after save
        res.send({
            result: "Done",
            data: data
        })
...
async function updateRecord(req, res) {
    try {
        ...
        error.keyValue ? errorMessage.name = "Maincategory With This Name Already Exist" : ""
        res.send({
            result: "Fail",
